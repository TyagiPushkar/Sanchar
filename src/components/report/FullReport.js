...
const styles = `
  ...
  /* Ensure proper stacking with header */
  .sticky-first-col thead tr:first-child th:first-child {
    z-index: 30;
  }
`;
useEffect(() => {
...
const getStatusValue = (loaNumber, checkpointId) => {
  const statusRecord = statusData.find(
    (record) => getTransactionValue(record, "589") === loaNumber,
  );
...
const getLoaValue = (loaNumber, checkpointId) => {
  const loaRecord = loaData.find(
    (record) => getLoaValueDirect(record, "60") === loaNumber,
  );
...
const calculatePendingAmount = (loaNumber) => {
  ...
  amountCheckpoints.forEach((amountCp, index) => {
    const amount =
      Number.parseFloat(getTransactionValue(statusRecord, amountCp)) || 0;
...
const getProjectStatus = (loaNumber) => {
  ...
  const warrantyEndDate = getDateFromValue(
    getTransactionValue(statusRecord, "592"),
  );
  const installationDate = getDateFromValue(
    getTransactionValue(statusRecord, "593"),
  );
  const completionDate = getDateFromValue(
    getTransactionValue(statusRecord, "625"),
  );
  ...
  if (
    (!installationDate || installationDate <= today) &&
...
return (
  <>
    ...
    <div className="material-list-container">
      ...
      <div
        className="table-container sticky-first-col"
